...
public class ResourceController {
    ...
    private static final Map<String, String> imageHeaders = Collections.unmodifiableMap(
    ...
    public CompletableFuture<HttpResponse> handle(HttpRequest request, HomekitClientConnection connection) throws IOException {
        if (POST != request.getMethod()) {
            return notFound();
        }

        try(ByteArrayInputStream bais = new ByteArrayInputStream(request.getBody())) {
            JsonObject json = Json.createReader(bais)
                    .readObject();

            int aid = json.getInt("aid");
            String resourceType = json.getString("resource-type");
            HomekitAccessory accessory = registry.getAccessories().stream()
            ...
            ResourceService resourceService = accessory.getServices().stream()
            ...
            CompletableFuture<byte[]> data = resourceService.handle(json.toString());
            if (data == null) {
                return notFound();
            }

            return data.thenApply(bytes -> {
                OkResponse ok = new OkResponse(bytes) {
                    @Override
